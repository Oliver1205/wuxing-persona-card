...
- **实时数据监控能力**：新增匿名 session、visitor、heartbeat 和 minute snapshot，管理端可查看当前在线用户、在线会话、今日 PV/UV、结果生成、分享点击、匹配入口、趋势曲线和实时事件流。
...
- **后台数据中台**：分页式运营看板，覆盖实时监控、核心概览、流量趋势、转化漏斗、结果排行、事件明细、性能压测、短链列表、CSV 导出和运行态状态。
