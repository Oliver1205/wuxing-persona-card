...
| 资深架构师 | `RESULT_VIEW` 读取链路不应等待非核心统计写入；正式高峰前必须观察队列积压和丢弃 | Phase 46 已将 `RESULT_VIEW` 改为异步，正式压测待执行 |
| 大厂面试官 | 不能说“已验证生产高并发”；需要准备迁移治理、短码冲突、异步事件丢失语义等追问 | 学习手册/后续任务待补 |
...
| P1 | showcase 截图墙改为“首页 -> 出生 -> 问答 -> 结果 -> 后台”5 步叙事 | 让普通人和面试官都能一眼看懂闭环 | docs-site 截图检查 |
| P1 | 面试学习手册补“短码冲突、迁移治理、异步事件丢失语义”任务卡 | 防止项目介绍被架构追问打穿 | 手册静态检查 |
| P2 | 将异步队列容量、批量大小、lastVisit 更新间隔配置化 | 生产演练时更容易调参和解释容量 | 后端单测 + 配置文档 |
