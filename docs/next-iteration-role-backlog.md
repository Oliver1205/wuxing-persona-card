...
| --- | --- | --- | --- | --- |
| P0 | 访问事件单机异步后仍缺少可配置延迟阈值 | 微信群或朋友圈集中打开短链时，需要在回归验证中及时发现 302 平均耗时退化 | `scripts/performance-smoke-test.sh`、`VisitEventService` | 已在 Phase 37 加入 `MAX_SHORTLINK_AVG_MS` / `MAX_ADMIN_AVG_MS` 可选阈值；后续再补 TP95 和事件丢弃计数输出 |
...
- 性能 smoke 可选低延迟阈值已在 Phase 37 落地，默认只输出耗时，设置阈值后可用于拦截明显退化。
