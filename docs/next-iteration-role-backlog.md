...
| 角色 | 新发现 | 状态 |
| --- | --- | --- |
| 产品经理 | 文档站“立即体验 H5”不应绕 README；首页和结果页 CTA 还可以进一步收束 | 待排期 |
| 普通访问者 | 前台应少说“短链”，多说“分享链接”；出生年月需要隐私和低成本解释 | Phase 47 已部分落地 |
| 前端交互 | 移动端出生步骤的底部操作区不应压住出生卡；自动下一题需要更可预期 | Phase 47 已部分落地 |
...
| 大厂面试官 | 不能说“已验证生产高并发”；需要准备迁移治理、短码冲突、异步事件丢失语义等追问 | Phase 51 已补短码冲突代码证据；迁移治理、外部短链一致性和异步丢弃语义仍待补强 |
...
| P0 | 文档站“立即体验 H5”改成更直接的本地/线上入口说明 | 作品集访客更容易从文档进入真实体验 | 静态链接检查 |
| P1 | 自动下一题增加更明确的倒计时/可取消表达，或改成手动为主 | 提升掌控感，降低误触焦虑 | Playwright 问答流截图 |
| P1 | 外部短链成功、本地绑定失败的补偿方案 | 防止 external 跨系统一致性被面试官追问打穿 | Provider 单测 + 面试问答 |
| P1 | 异步事件丢弃语义补单测和 smoke 阈值 | 证明低延迟不是靠不可见丢事件换来 | VisitEventServiceTest + performance smoke |
