...
状态：v0.5 已完成短链列表 PV / UV / UIP 读取和短链访问明细接入。
...
v1.2-v1.4 面试表达可以继续升级为：

> 我进一步给 external 接入补了运行态治理和工程质量保障：后台能查看 external 模式、统计开关、fallback、domain、groupId 和可选连通性探测；GitHub Actions 自动跑质量门禁；Docker smoke 脚本能验证部署后的主链路；Testcontainers profile 用真实 MySQL schema 补充集成测试。这让短链接入不只是代码适配，而是具备上线前后可验证、可排查的工程闭环。
