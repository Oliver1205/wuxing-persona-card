...
- 当前后台接口统一要求 `X-Admin-Token`，并使用常量时间比较减少简单时序侧信道。
- 集成测试覆盖 overview、短链列表、CSV 导出、短链访问明细、external 状态、访问事件 runtime 和手动聚合接口的未授权 401。
- 这适合单人作品集和演示环境，能避免后台数据裸奔。

边界要主动说：

- 这不是 RBAC、审计日志、账号体系或细粒度权限。
- 如果进入真实多人运营，应补登录态、角色权限、操作审计、token 轮换和更严格的网关/内网访问控制。

## 8. 为什么还没有 Flyway 或 Liquibase？
